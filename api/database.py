"""Database layer for the reports service.

Credits (the ``users`` table) live in the shared Railway Postgres so this
service and trading_agents spend from one wallet.  If ``DATABASE_URL`` is
unset the module falls back to SQLite for local development.

Service-specific tables (``packs``, ``purchases``) stay in SQLite on the
Railway volume, which keeps the shared Postgres to just users + credits.
"""
...
def init_db() -> None:
    ...
    with _sqlite_conn() as conn:
        conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_packs_status ON packs(status, created_at);
...
def deduct_credits(user_id: str, amount: int) -> bool:
    ...
    Returns False without changing anything when the balance is too low.
    The guard lives in the WHERE clause so concurrent requests cannot drive
    a balance negative.
    """
    if _USE_POSTGRES:
        with _pg_conn() as conn:
...
# ── Packs (service SQLite) ───────────────────────────────────────────────────

def _row_to_pack(row: sqlite3.Row) -> Dict[str, Any]:
    pack = dict(row)
    pack["report_keys"] = json.loads(pack["report_keys"]) if pack.get("report_keys") else {}
    return pack


def create_pack(snapshot_date: str) -> str:
    """Insert a pack in the ``building`` state and return its id."""
    pack_id = str(uuid.uuid4())
    with _sqlite_conn() as conn:
        conn.execute(
            "INSERT INTO packs (id, snapshot_date, status, created_at) VALUES (?, ?, 'building', ?)",
            (pack_id, snapshot_date, _now()),
        )
    return pack_id
        ...
        conn.execute(
            """UPDATE packs
               SET status = 'ready', report_keys = ?, data_through = ?,
                   ticker_count = ?, completed_at = ?
               WHERE id = ?""",
            (json.dumps(report_keys), data_through, ticker_count, _now(), pack_id),
        )


def mark_pack_failed(pack_id: str, error_message: str) -> None:
    with _sqlite_conn() as conn:
        conn.execute(
            "UPDATE packs SET status = 'failed', error_message = ?, completed_at = ? WHERE id = ?",
            (error_message[:2000], _now(), pack_id),
        )


def get_latest_ready_pack() -> Optional[Dict[str, Any]]:
    """The newest pack users can buy, or None if nothing has built yet."""
    with _sqlite_conn() as conn:
        row = conn.execute(
            "SELECT * FROM packs WHERE status = 'ready' ORDER BY completed_at DESC LIMIT 1"
        ).fetchone()
...
def list_packs(limit: int = 50) -> List[Dict[str, Any]]:
    with _sqlite_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM packs ORDER BY created_at DESC LIMIT ?", (limit,)
        ).fetchall()
        return [_row_to_pack(r) for r in rows]


def has_pack_for_date(snapshot_date: str) -> bool:
    """True if a pack for this snapshot date is already ready or building."""
    with _sqlite_conn() as conn:
        row = conn.execute(
            """SELECT 1 FROM packs
...
def reap_stale_building_packs() -> int:
    """Fail any pack left ``building`` by a crash or redeploy.
...
def get_purchase(user_id: str, pack_id: str) -> Optional[Dict[str, Any]]:
    with _sqlite_conn() as conn:
        row = conn.execute(
            "SELECT * FROM purchases WHERE user_id = ? AND pack_id = ?",
            (user_id, pack_id),
        ).fetchone()
...
def list_user_purchases(user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    """A user's purchases joined with their pack, newest first."""
    with _sqlite_conn() as conn:
        rows = conn.execute(
...
class InsufficientCredits(Exception):
    """Raised when a user cannot afford a pack."""


def purchase_pack(user_id: str, pack_id: str, cost: int) -> tuple[Dict[str, Any], bool]:
    """Charge a user for a pack, exactly once.

    Returns ``(purchase, charged)``.  ``charged`` is False when the user
    already owned this pack, which makes the endpoint safe to retry and
    ...
    """
    existing = get_purchase(user_id, pack_id)
    if existing:
    ...
    try:
        with _sqlite_conn() as conn:
            conn.execute(
                """INSERT INTO purchases (id, user_id, pack_id, credits_spent, created_at)
                   VALUES (?, ?, ?, ?, ?)""",
                (purchase_id, user_id, pack_id, cost, created_at),
            )
    except sqlite3.IntegrityError:
        # Two requests raced. The other one won, so give this charge back.
        add_credits(user_id, cost)
        winner = get_purchase(user_id, pack_id)
        if winner:
            return winner, False
        raise
    except Exception:
