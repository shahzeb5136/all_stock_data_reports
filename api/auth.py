...
PyJWKClient caches the public keys so this does not hit the network on
every request.

This mirrors the auth layer in the trading_agents service on purpose: both
products sit behind the same Clerk application and the same user IDs, which
is what lets them share one credit wallet.
"""
