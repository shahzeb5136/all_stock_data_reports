...
that have dipped the most recently, fetches basic company info, and produces
a polished PDF report.

Methodology
-----------
For each ticker we compute several dip metrics over the trailing ~22 trading days:
  1. Drawdown from 30-day high: (recent_price - 30d_high) / 30d_high
  ...
  3. Z-score of the 30-day return vs the ticker's own rolling 30-day return
     distribution (measures how abnormal the dip is relative to the stock's
     own historical behaviour).
  4. Percentage below the 60-day moving average.

These are combined into a composite "dip score" that ranks tickers.

Long-Term Trend Filter
----------------------
After scoring, stocks are filtered to exclude those in sustained long-term
downtrends. Only stocks where at least one of the 1-year or 1.5-year returns
is positive are kept. This ensures we surface momentary dips in otherwise
healthy stocks, not stocks that are simply declining.
"""
...
# Tickers the downloader could not lift off a stale split adjustment. Dropping
# them is the last line of defence: an unrepaired split leaves a one-day -50%
# cliff that scores as the single biggest dip in the universe. Guarded, and
# path-inserted, so a direct `python reports/dip_analyzer.py` still runs.
...
def analyze_dips(df, lookback=22):
    """
    For each ticker compute dip metrics over the last `lookback` trading days,
    plus long-term return metrics (1-year, 1.5-year).
    Returns a DataFrame ranked by composite dip score (most negative = biggest dip).
    """
    results = []
    ...
    for ticker, grp in df.groupby("ticker"):
        ...
        # 1. Drawdown from 30-day high
        high_30d = recent["close"].max()
        drawdown_from_high = (current_price - high_30d) / high_30d
        ...
        # 4. % below 60-day moving average
        grp["ma60"] = grp["close"].rolling(60).mean()
        ma60_val = grp["ma60"].iloc[-1]
        pct_below_ma60 = (current_price - ma60_val) / ma60_val if pd.notna(ma60_val) else 0

        # 5. Composite score (all components are negative for dips — more negative = bigger dip)
        composite = (
            0.30 * drawdown_from_high +
            0.30 * return_30d +
        ...
        results.append({
            ...
            "pct_below_ma60": round(pct_below_ma60 * 100, 2),
            "composite_dip_score": round(composite * 100, 4),
    ...
    results_df.sort_values("composite_dip_score", ascending=True, inplace=True)
    results_df.reset_index(drop=True, inplace=True)
    return results_df


def filter_long_term_downtrends(dip_df, top_n=20):
    """
    Filter out stocks that are in a sustained long-term downtrend.
    Keeps stocks where AT LEAST ONE of:
      - 1-year return is positive, OR
      - 1.5-year return is positive, OR
    ...
    pool = dip_df.head(top_n * 3).copy()

    def passes_filter(row):
        r1y = row["return_1y_pct"]
        r1_5y = row["return_1_5y_pct"]
        # If both are available and both negative → long-term downtrend → exclude
        if pd.notna(r1y) and pd.notna(r1_5y):
            return r1y > 0 or r1_5y > 0
    ...
    print(f"  Long-term trend filter: removed {removed_count} stocks in sustained downtrends")
...
def generate_mini_charts(df, tickers, output_dir="charts"):
    ...
    for ticker in tickers:
        ...
        for cfg in CHART_CONFIGS:
            ...
            color = "#d32f2f" if prices.iloc[-1] < prices.iloc[0] else "#2e7d32"
            ...
            path = os.path.join(output_dir, f"{ticker}_{cfg['key']}.png")
...
def generate_pdf(top20_df, company_info, price_df, output_path="zz_dip_report.pdf"):
    ...
    section_label = ParagraphStyle(
        ...
        fontSize=10, textColor=HexColor("#0f3460"),
    ...
    story.append(Paragraph("Stock Dip Report", title_style))
    story.append(Paragraph(
        f"Top {len(top20_df)} Biggest Recent Dips (Long-Term Healthy) &mdash; Generated {top20_df['latest_date'].iloc[0].strftime('%B %d, %Y')}",
        subtitle_style
    ))
    ...
    story.append(Paragraph(
        "This report identifies stocks with the largest recent price dips using a composite score "
        "that blends four metrics: (1) drawdown from the trailing 30-day high, (2) 30-day simple return, "
        "(3) z-score of that return relative to the stock's own historical volatility, and "
        "(4) percentage below the 60-day moving average. "
        "<br/><br/>"
        "<b>Long-Term Trend Filter:</b> Stocks in sustained downtrends are excluded. Only stocks where "
        "at least one of the 1-year or 1.5-year returns is positive are included. This ensures we "
        "surface momentary dips in otherwise healthy stocks — not declining companies. "
        "A more negative composite score indicates a deeper, more statistically unusual dip. "
    ...
    # Summary table — now includes long-term returns
    story.append(Paragraph("Summary Table", section_label))
    summary_data = [["#", "Ticker", "Price", "30d Ret", "Drawdown", "1Y Ret", "1.5Y Ret", "Score"]]
    for i, row in top20_df.iterrows():
        ...
        summary_data.append([
            ...
            f"{row['drawdown_from_high_pct']:.1f}%",
            r1y,
            r1_5y,
            f"{row['composite_dip_score']:.2f}",
        ])
    ...
    for idx, row in top20_df.iterrows():
        ...
        # Key metrics table — now with long-term returns
        ...
        metrics_data = [
            ["Current Price", "30-Day Return", "Drawdown (30d High)", "Dip Score"],
            [f"${row['current_price']:.2f}", f"{row['return_30d_pct']:.1f}%",
             f"{row['drawdown_from_high_pct']:.1f}%", f"{row['composite_dip_score']:.2f}"],
            ["1-Year Return", "1.5-Year Return", "Z-Score", "% Below 60d MA"],
            [r1y_display, r1_5y_display, f"{row['z_score']:.2f}", f"{row['pct_below_ma60']:.1f}%"],
        ...
        mt.setStyle(TableStyle([
            ...
            ("BACKGROUND", (0, 2), (-1, 2), HexColor("#e8eef5")),
            ("BACKGROUND", (0, 4), (-1, 4), HexColor("#e8eef5")),
            ("BACKGROUND", (0, 6), (-1, 6), HexColor("#e8eef5")),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
...
if __name__ == "__main__":
    print("=" * 60)
    print("  STOCK DIP ANALYZER")
    print("  (with Long-Term Trend Filter)")
    print("=" * 60)
    ...
    # Step 2: Analyze dips
    print("\n[2/5] Analyzing dips (composite scoring + long-term returns)...")
    results = analyze_dips(df, lookback=22)
    print(f"  Analyzed {len(results)} tickers")

    # Step 3: Filter out long-term downtrends
    print("\n[3/5] Filtering out stocks in long-term downtrends...")
    top20 = filter_long_term_downtrends(results, top_n=20)

    print(f"\n  Top {len(top20)} dips (long-term healthy stocks only):")
    for i, row in top20.iterrows():
        ...
        print(f"    {i+1:>2}. {row['ticker']:<6} | 30d: {row['return_30d_pct']:>7.1f}% | "
              f"1Y: {r1y} | 1.5Y: {r1_5y} | score: {row['composite_dip_score']:.2f}")
